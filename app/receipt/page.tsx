...
function PerforatedEdge({ position }: { position: 'top' | 'bottom' }) {
  return (
    <div
      ...
      style={{
        paddingTop: position === 'top' ? 0 : undefined,
        paddingBottom: position === 'bottom' ? 0 : undefined,
        height: '13px',
        overflow: 'hidden',
      }}
    >
...
export default function ReceiptPage() {
  ...
  const handleShare = async () => {
    const text = [
      ...
      `Bank: ${receipt.recipientBank} | ${receipt.recipientAccount}`,
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: 'PalmPay Receipt', text });
      } catch (err) {
        // AbortError = user dismissed the share sheet — not a real error, ignore it.
        // Anything else (e.g. NotAllowedError) falls through to clipboard.
        if (err instanceof Error && err.name === 'AbortError') return;
        // Fallback to clipboard on any other share failure
        try {
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col items-center py-8 px-4">
      ...
      <div className="w-full max-w-md flex items-center justify-between mb-5">
        ...
        <div className="flex gap-2">
          <button
            onClick={handleShare}
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-white border border-gray-200 text-xs font-bold text-[#2A245A] shadow-sm hover:shadow transition-shadow"
          >
        ...
        <div className="relative px-7 pt-5 pb-7">
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-2 gap-3 items-start">
              <p className="text-[19px] font-extrabold" style={{ color: '#2A245A' }}>
                Recipient:
              </p>
